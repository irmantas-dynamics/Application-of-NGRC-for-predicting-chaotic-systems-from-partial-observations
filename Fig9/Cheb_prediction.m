...
%     W = (pinv(RR)*B).';
    ...
    if plt ==1
        ...
        legend({'actual','learned'})
        subplot(212)
        plot(TT,abs(D-W*R),'-')
        ylabel('$\dot{x}-\dot{x}_{pred}$','Interpreter','latex')
...
%     Y1 = Xpred /M;
    ...
    if plt == 1
        ...
        indxT = find(skirt(idx:end)>0.01,1);
        if ~isempty(indxT)
            fprintf("Corect prediction time (multiplied by Lyap): %f\n\n",TP(indxT+idx-1)/CC.lyapTime)
        end
