function [ErrorL,Terr,signal,XP,TP,R,RR,D,W]=Monom_prediction(PredictTime,X0,hT,CC,bet,Xpred,Tpred,plt)
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % 
% Monom_prediction() from signal X0 makes prediction by using NVAR with
% polynomes. Function also compares its predictions with signal
...
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % 
    ...
    %Monomials nth-order, for k variables
    fnname = sprintf('Monom_Rn_%d',k);
    ...
    Monom = str2func(fnname);

    %array of feature vectors
    R =features(Monom,X,polyn_deg,dtrvec,dtot,szL);
...
%     W = (pinv(RR)*B).';
    ...
    for j=(k-1)*Ntau+1:Np-1
        ...
        r =features(Monom,x,polyn_deg,dtrvec,dtot,1);
        XP(j+1)=x(1)+h*(W*r);
    end
